class Main {
  ...
  }//end recursive implementation method
  //helper method for finding the miminum of the unsorted array
  public static int minIndex (int[] a, int i, int j)
  {
    ...
    return (a[i] < a[k]) ? i : k;
    /* conditional equivalent to this return statement
    if (a[i] < a[k])
      return i;
    else
      return k;
      */
  }//end recursive minimum finder method
  ...
  public static void sort(int a[])
  {
    ...
    //this also separates between the sorted and unsorted subarrays
    //don't need to sort the last element
    for( int i = 0; i < n-1; i++)
    {
       //find the minimum of the unsorted subarray
       int minIndex = i;
       for (int j = i + 1; j < n; j++)
       {
        if (a[j] < a[minIndex])
